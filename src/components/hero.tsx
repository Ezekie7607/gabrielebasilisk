...
export function Hero() {
  ...
  return (
    <section id="top" className="relative min-h-svh overflow-hidden">
      ...
      <div className="relative z-10 mx-auto flex min-h-svh max-w-5xl flex-col justify-end px-4 pb-20 pt-24 sm:px-6 sm:pb-14">
        <div id="hero-copy" className="hero-flow relative">
          <p className="stagger-in font-pixel text-micro tracking-widest text-muted">{t.hero.kicker}</p>
          <p className="font-script stagger-in mt-5 text-[clamp(1.55rem,4vw,2.5rem)] text-fg">
            {t.hero.motto}
          </p>
          <h1 className="stagger-in -mt-1 max-w-3xl font-display text-display leading-display text-fg sm:-mt-2">
            {t.hero.h1a}
            <br />
            {t.hero.h1b}
            <br />
          ...
          <dl className="mt-10 grid grid-cols-2 gap-6 sm:mt-14 sm:grid-cols-4">
            {t.stats.map((stat) => (
